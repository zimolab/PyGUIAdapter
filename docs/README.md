...
虽然`PyGUIAdapter`对程序中的未捕获异常进行了处理，但是尽力预见并避免可能发生的异常仍然应当作为一种共识，因为这是提高程序健壮性、稳定性的基础之一。就经验而言，程序中的异常或错误，很多时候都源自于未经检验的用户输入，**“永远不要相信用户的输入”**应当成为所有开发者的金玉良言。

`PyGUIAdapter`为函数参数的校验提供了一种简单直接的机制：对于非法的参数，请抛出`ParameterError`。
