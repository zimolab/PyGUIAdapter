...
上面的示例演示了`PyGUIAdapter`的基本用法，但`PyGUIAdapter`的功能远不止这些，下面通过一个更加实际的例子来展示`PyGUIAdapter`的更多特性。

这次，让我们来实现一个二维码生成器，其基本功能是将用户输入的文本编码成二维码，并保存为本地图片。同时，允许用户调整二维码的尺寸、边框宽度、颜色、背景色、
纠错率等属性。

> 在本示例中，二维码的生成用到了`qrcode`、`Pillow`等第三方库，因此，如要运行示例代码，请确保已提前安装好相关依赖，如：
> 
> ```shell
...
前面说过，`PyGUIAdapter`允许开发者聚焦业务逻辑，因此，开发者拿到需求后要考虑的第一件事，永远应该是这个需求如何实现。其中，确定需要用户提供的数据及
其类型，又是优先事项中的优先事项。

对于本示例而言，我们假设要将核心业务逻辑封装到`make_qrcode()`函数中，然后根据需求，枚举可供用户调整的参数，并确定其类型：

- `text`：代编的文本，类型为`str`。
- `box_size`：码元大小，类型为`int`。
- `border`：二维码的边框宽度，类型为`int`。
- `color`：二维码的颜色，类型为`str`。
- `background`：二维码的背景色，类型为`str`。
- `error_correction`：二维码的纠错率，类型为`str`。
- `save_path`：二维码的保存路径，类型为`str`。
- `allow_overwrite`：是否允许覆盖已存在的二维码文件，类型为`bool`。
...
确定了函数签名，接下来就可以实现核心功能——生成二维码。 由于我们的目的并非讲解`qrcode`的使用，所以这一部分就不详细展开，大家也不必深究，大家只要能够理解：
这段代码的主要功能是，使用`qrcode`库，根据`box_size`、`border`、`color`、`background`、`error_correction`等参数，将`text`编码成二维码图片，
并将其保存到`save_path`中。
