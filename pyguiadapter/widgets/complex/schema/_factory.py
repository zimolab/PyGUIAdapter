...
from ..commons import AlreadyExistError, NotFoundError


class ObjectSchemaFactory(object):
    def __init__(self):
        self._registry: Dict[type, ValueTypeBase] = {}

    def register(self, typ: type, vt: ValueTypeBase):
        if not isinstance(typ, type):
            raise TypeError(f"typ must be a type, got {type(typ)}")

        if not isinstance(vt, ValueTypeBase):
    ...
    def unregister(self, typ: type) -> ValueTypeBase:
    ...
    def get_value_type(self, typ: type) -> ValueTypeBase:
        if not typ in self._registry:
            raise NotFoundError(f"type '{typ}' not registered yet")
        return self._registry[typ]

    def create_schema(
        self, schema_dict: Dict[str, Union[ValueTypeBase, type, str, Any]]
    ) -> Dict[str, ValueTypeBase]:
        schema = {}
        for key, type_or_value in schema_dict.items():
            if isinstance(type_or_value, ValueTypeBase):
